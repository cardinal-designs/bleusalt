...
class CartDrawer extends HTMLElement {
    ...
    quickAddClick(cardElement, event) {
      ...
      // Remove the 'selected' attribute from all quick-add buttons
      cardElement.querySelectorAll(".quick-add__button").forEach((button) => button.removeAttribute("selected"));
    
      // Set the 'selected' attribute on the clicked button
      event.target.setAttribute("selected", true);
    
      let e = [];
      // Collect all selected values from the buttons
      cardElement.querySelectorAll(".quick-add__button[selected]").forEach((button) => {
        e.push(button.value);
      });
    
      const i = e.length > 1 ? e.reverse().join(" / ") : e[0];
      ...
      // Check if any variant matches the selected option
      cardVariants.forEach((variant) => {
        if (variant.title === i) {
          console.log("✅ Match found:", variant.title);
          matchFound = true;
          return;
        } else {
          console.log("❌ No match for:", variant.title);
        }
      });
      
      if (!matchFound) {
        console.log("⚠️ No matching variant found for:", i);
      }
    }
