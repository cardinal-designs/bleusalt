...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!customElements.get('quick-add')) {
    customElements.define('quick-add', class QuickAdd extends HTMLElement {
      ...
      sizeAvailabilities() {
        if (!this.selectedColor) return;
        this.variants.forEach((variant) => {
          if (variant.options[this.colorIndex] !== this.selectedColor.value) return;
          let sizeButton = this.querySelector(`button[value="${variant.options[this.sizeIndex]}"]`);
          if (sizeButton) sizeButton.disabled = !variant.available;
        });
      }
      ...
      changeImage(variant) {
        ...
        this.json.media.forEach((media) => {
          ...
          if (mediaAlt !== this.selectedColor.value) return;
          if (!foundOne && !foundTwo) {
            updateSrcSet(firstImage, media.src);
            foundOne = true;
          } else if (foundOne && !foundTwo) {
  ...
  // Loop through all <quick-add> elements and upgrade them
  const quickAddElements = document.querySelectorAll('quick-add');
  quickAddElements.forEach((el) => {
    if (!(el instanceof customElements.get('quick-add'))) {
