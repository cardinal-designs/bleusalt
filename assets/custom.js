...
window.addEventListener('scroll', function () {
  ...
  if (window.scrollY > 600) {
    if(header)header.classList.add('sticky-header-wrapper');
    if(index)index.classList.add('sticky-header-enabled');
  } else {
    if(header)header.classList.remove('sticky-header-wrapper');
    if(index)index.classList.remove('sticky-header-enabled');
  }
});
